...
- `/items` -- create new item / get all items;
- `/items/{itemId}` -- get / update / delete item by provided id;

## Create a new item

Create a new item by sending a `POST` request to `/items` with properties `itemName`, `imageUrl`, `description` 
